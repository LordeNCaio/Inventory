...
public class Inventory {
	...
	public Inventory(int lines, int columns, int maxItemPerSlot) {
		...
		if(this.maxSlotsX < 1) {this.maxSlotsX = 1;}
		if(this.maxSlotsY < 1) {this.maxSlotsY = 1;}
		...
		for(int i = slots.size()-1; i > -1 ; i--) {
			...
			if(slot.getCurItem() == item) {
				if(amount <= slot.getAmount()) {
					...
					if(slot.getAmount() <= 0) {slot.setCurItem(null);}
					return;
				}
				else {
	...
	public void update() {
		if(slotU) {slotU = false; curSlotY--; if(curSlotY < 0) {curSlotY = maxSlotsY-1;}}
		if(slotD) {slotD = false; curSlotY++; if(curSlotY > maxSlotsY-1) {curSlotY = 0;}}
		if(slotL) {slotL = false; curSlotX--; if(curSlotX < 0) {curSlotX = maxSlotsX-1;}}
		if(slotR) {slotR = false; curSlotX++; if(curSlotX > maxSlotsX-1) {curSlotX = 0;}}		
	}	
	...
	public void render(int x, int y, int boxSize, Graphics g) {
		...
		for(int i = 0; i < slots.size(); i++) {
			Slot slot = slots.get(i);
			loopX++;if(loopX > maxSlotsX-1) {loopX = 0; loopY++; if(loopY > maxSlotsY-1) {loopY = 0;}}
			...
			if(slot.getCurItem() != null) {
				g.drawImage(slot.getCurItem().getSprite(), x + (boxSize*loopX), y + (boxSize*loopY), boxSize, boxSize, null);
				g.setColor(Color.WHITE);
				g.setFont(new Font("Arial", Font.BOLD, boxSize/4));
				g.drawString(slot.getAmount()+"/"+slot.getMaxAmount(), x + (boxSize*loopX) + boxSize/4, y + (boxSize*loopY) + boxSize - 5);
			}				
		}		
		g.setColor(Color.WHITE);
